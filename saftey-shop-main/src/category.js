...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    const filtered = categorySlug
      ? products.filter(p => p.category && p.category.slug === categorySlug)
      : products;

    // عنوان دسته‌بندی
    categoryTitleEl.textContent = filtered.length
      ? filtered[0].category.name   // حالا از مدل Category می‌گیریم
      : 'همه محصولات';
    ...
    productListEl.innerHTML = '<p class="col-span-full text-center text-red-500">مشکلی در بارگذاری محصولات پیش آمد.</p>';
  }
});
